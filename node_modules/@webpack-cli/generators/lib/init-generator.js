...
class InitGenerator extends types_1.CustomGenerator {
    ...
    async prompting() {
        if (!fs_1.existsSync(this.resolvedGenerationPath)) {
            this.utils.logger.log(`${colorette_1.blue("ℹ INFO ")} supplied generation path doesn't exist, required folders will be created.`);
            try {
    ...
    writing() {
        this.utils.logger.log(`${colorette_1.blue("ℹ INFO ")} Initialising project...`);
        ...
        try {
            ...
            const source = fs_2.readFileSync(this.configurationPath, {
                encoding: "utf8",
            });
            const formattedSource = prettier.format(source, {
                parser: "babel",
            });
            fs_2.writeFileSync(this.configurationPath, formattedSource);
        }
        catch (err) {
            this.utils.logger.log(`${colorette_1.yellow(`⚠ Generated configuration may not be properly formatted as prettier is not installed.`)}`);
            return;
        }
    }
}
